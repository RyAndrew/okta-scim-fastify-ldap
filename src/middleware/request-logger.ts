// ---------------------------------------------------------------------------
// Round-trip HTTP request/response logger (identical contract to PS app)
// ---------------------------------------------------------------------------
...
export function registerRequestLogger(server: FastifyInstance, db: Knex): void {
  ...
  server.addHook('onSend', async (request, _reply, payload) => {
    ...
    if (typeof payload === 'string') responseBodyStr = payload;
    else if (Buffer.isBuffer(payload)) responseBodyStr = payload.toString('utf8');
  ...
  server.addHook('onResponse', async (request, reply) => {
    ...
    db('incoming_requests')
      ...
      .update({
        ...
        response_body: entry.responseBody ? entry.responseBody.substring(0, 65_535) : null,
