...
import { LdapBackend } from './backend';

// ---------------------------------------------------------------------------
// SCIM discovery (RFC 7644 §4) — unauthenticated
// ---------------------------------------------------------------------------
...
async function start(): Promise<void> {
  ...
  // ── LDAP connection ───────────────────────────────────────────────────────
  const ldapClient = new LdapClient(config.ldap, db);
  await ldapClient.connect();
  server.log.info('LDAP connection established');

  // Re-bind on close (best-effort keepalive is the responsibility of ldapts)
  server.addHook('onClose', async () => {
    await ldapClient.disconnect().catch(() => undefined);
  });
  ...
  const backend = new LdapBackend(ldapClient, config);
  await server.register(createScimUsersPlugin(backend), { prefix: '/scim/v2/Users' });

  server.get('/health', async (_request, reply) => {
    return reply.status(200).send({ status: 'ok', service: 'scim-ldap-bridge' });
  });
